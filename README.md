...
2. 在本機專案目錄執行（將 `YOUR_USER` / `toeic-aquarium` 改成你的）：
   ```bash
   git init
   git add index.html game.js style.css words.json icon.png manifest.webmanifest .nojekyll .gitignore
   git add toeic_suffix_words_1000.json csv_excel_to_words_json.py README.md Operator WI.md
   git commit -m "Initial game for GitHub Pages"
   git branch -M main
